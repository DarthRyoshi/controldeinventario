...
class PrestamoController {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($prestamo_id) {
            // Solo registramos el prestamo_id en bitacora
            $stmt = $conn->prepare(
                "INSERT INTO bitacora (prestamo_id, accion, fecha) VALUES (?, 'CREACIÓN', NOW())"
            );
...
case 'devolverPrestamo':
    ...
    if ($id) {
        if ($prestamoModel->devolver($id)) {
            // Solo registramos el prestamo_id en bitacora
            $stmt = $conn->prepare(
                "INSERT INTO bitacora (prestamo_id, accion, fecha) VALUES (?, 'DEVOLUCIÓN', NOW())"
            );
            $stmt->execute([$id]);
        }
    }
